...
export function useSessions(limit = 20, offset = 0) {
  return useQuery({
    ...
    queryFn: () => api.get<SessionList>(`/api/sessions?limit=${limit}&offset=${offset}`),
  })
}
...
export function useCreateSession() {
  ...
  return useMutation({
    mutationFn: (body: SessionCreateBody) => api.post<SessionCreated>('/api/sessions', body),
    onSuccess: () => {
...
export function useContinueSession() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ sessionId, body }: { sessionId: string; body: SessionContinueBody }) =>
...
export function useSubmitFeedback() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, feedback }: { id: string; feedback: FeedbackStatus; sessionId: string }) =>
      api.patch<Recommendation>(`/api/recommendations/${id}/feedback`, { feedback }),
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: keys.session(vars.sessionId) })
    },
  })
